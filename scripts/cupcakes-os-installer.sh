#!/usr/bin/env bash
# Cupcakes OS Installer — EVEREST 4.0 Edition
...
draw_logo() {
    printf '  %bCUPCAKES_OS OS%b  %b▸%b  %bEVEREST 4.0%b\n' \
...
tab_header() {
    ...
    printf '\n'
    printf '  %b┌────────────────────────────────────────────────────────┐%b\n' "$CF" "$R"
    printf '  %b│%b  %bCUPCAKES_OS OS%b  %b▸%b  EVEREST 4.0%b                              %b│%b\n' \
...
step_confirm() {
    while true; do
        ...
        menu "Ready to install?" \
            "Install now|Erase ${disk} and install Cupcakes OS EVEREST 4.0" \
...
{ pkgs, lib, ... }:
{
  system.nixos.variantName = "Cupcakes OS EVEREST 4.0 ${desktop_label} Edition";
...
draw_install_title() {
    printf '  %b┌────────────────────────────────────────────────────────┐%b\n' "$CF" "$R"
    printf '  %b│%b  %bCUPCAKES_OS OS%b  %b▸%b  Installing EVEREST 4.0%b                  %b│%b\n' \
        "$CF" "$R" "${B}${CW}" "$R" "${D}${CG}" "$R" "${D}${CG}" "$CF" "$R"
    printf '  %b└────────────────────────────────────────────────────────┘%b\n' "$CF" "$R"
}
...
run_install() {
    ...
    printf '\n'
    draw_install_title
    printf '  %bInstalling Cupcakes OS EVEREST 4.0%b\n' "$CC" "$R"
    printf '  %bLog: %s%b\n' "${D}${CG}" "$install_log" "$R"
    printf '\n'
...
page_done() {
    ...
    printf '  %b│%b  %b✓  Installation Complete%b                               %b│%b\n' \
        "$CF" "$R" "${B}${CP}" "$R" "$CF" "$R"
    printf '  %b│%b  %bCupcakes OS EVEREST 4.0 is installed%b                     %b│%b\n' \
        "$CF" "$R" "$CS" "$R" "$CF" "$R"
    printf '  %b└────────────────────────────────────────────────────────┘%b\n' "$CF" "$R"
    printf '\n'
